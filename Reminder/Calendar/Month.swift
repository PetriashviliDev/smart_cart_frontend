...
struct Month: Identifiable, Equatable {
    ...
    init(from date: Date, order: Order) {
        ...
        var weeks: [Week] = [Week(days: currentWeekDays, order: .current)]
...
extension Month {
    var previousMonth: Month? {
        guard let previousMonthDate = Calendar.current.date(byAdding: .month, value: -1, to: initializedDate)
        else {
            return nil
        }
    ...
    var nextMonth: Month? {
        guard let nextMonthDate = Calendar.current.date(byAdding: .month, value: 1, to: initializedDate)
        else {
            return nil
        }
